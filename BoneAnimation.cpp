...
BoneAnimation::BoneAnimation(std::string const& filename) {
	...
	uint32_t frames = (uint32_t) frame_bones.size() / (uint32_t) bones.size();
	...
	{ //read actual mesh:
		...
		static_assert(sizeof(Vertex) == 3 * 4 + 3 * 4 + 4 * 1 + 4 * 4 + 4 * 4, "Vertex is packed.");
		...
		for (auto const& vertex : data) {
			//std::cout << glm::to_string(vertex.BoneWeights) << std::endl << glm::to_string(vertex.BoneIndices) << std::endl << std::endl;
		...
		Position = MeshBuffer::Attrib(3, GL_FLOAT, GL_FALSE, sizeof(Vertex), offsetof(Vertex, Position));
		Normal = MeshBuffer::Attrib(3, GL_FLOAT, GL_FALSE, sizeof(Vertex), offsetof(Vertex, Normal));
		Color = MeshBuffer::Attrib(4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(Vertex), offsetof(Vertex, Color));
		//TexCoord = MeshBuffer::Attrib(2, GL_FLOAT, GL_FALSE, sizeof(Vertex), offsetof(Vertex, TexCoord));
		BoneWeights = MeshBuffer::Attrib(4, GL_FLOAT, GL_FALSE, sizeof(Vertex), offsetof(Vertex, BoneWeights));
		BoneIndices = MeshBuffer::Attrib(4, GL_UNSIGNED_INT, GL_FALSE, sizeof(Vertex), offsetof(Vertex, BoneIndices));
...
void BoneAnimationPlayer::set_uniform(GLint bones_mat4x3_array) const {
	...
	for (uint32_t b = 0; b < bones.size(); ++b) {
		...
		//if (b == 0)
		//	std::cout << "Bone " << b << " " << glm::to_string(pose_bone.position) << std::endl;

		glm::mat3 r = glm::mat3_cast(pose_bone.rotation);
		//glm::mat3 r = glm::mat3_cast(glm::quat(1.0f, 0.0f, 0.0f, 0.0f));
		...
		//std::cout << glm::to_string(rs) << std::endl;
		glm::mat4x3 trs = glm::mat4x3(
			rs[0], rs[1], rs[2], pose_bone.position
		);
		/*glm::mat4x3 trs = glm::mat4x3(
			rs[0], rs[1], rs[2], glm::vec3(0.0f, 0.0f, 0.0f)
		);*/
		//std::cout << glm::to_string(trs) << std::endl;

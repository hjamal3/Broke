...
BoneVertexColorProgram::BoneVertexColorProgram() {
	program = gl_compile_program(
		...
		//"	highp uint uzero = uint(0);\n"
		//"	highp uint u20 = uint(1073000000);\n"
		//"	color = Color;\n"
		//"   color = vec4(abs(BoneWeights.x+BoneWeights.y+BoneWeights.z+BoneWeights.w - 1.0) * 100.0, 0.0, 0.0, 1.0);\n"
		//"   color = vec4(BoneIndices.x / 18.0, 0.0, 0.0, 1.0);\n"
		//"	color = vec4(0.0, 0.0, 0.0, 1.0);\n"
		//"	if (BoneIndices.x == uzero) {\n"
		//"		color = vec4(0.0, 1.0, 0.0, 1.0);\n"
		//"	}\n"
		//"	color = vec4(0.0, BoneWeights.x, 0.0, 1.0);\n"
		"}\n"
		,
		"#version 330\n"
...
Load< BoneVertexColorProgram > bone_vertex_color_program(LoadTagEarly, []() {
